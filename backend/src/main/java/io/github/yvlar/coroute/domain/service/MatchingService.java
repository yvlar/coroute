...
import io.github.yvlar.coroute.dto.request.MatchingRequest;
...
public class MatchingService {
  ...
  private boolean aJoursCompatibles(final Trajet trajet, final List<JourSemaine> joursRecherches) {
    if (joursRecherches == null || joursRecherches.isEmpty()) {
      return true;
    }
    return TrajetType.PONCTUEL.equals(trajet.getType())
        || trajet.getJoursRecurrence().stream().anyMatch(joursRecherches::contains);
  }

  private List<JourSemaine> calculerJoursCompatibles(
      final Trajet trajet, final List<JourSemaine> joursRecherches) {
    if (joursRecherches == null
        || joursRecherches.isEmpty()
  ...
  private int calculerScore(final Trajet trajet, final List<JourSemaine> joursRecherches) {
    int score = 0;
    ...
    if (joursRecherches != null && !joursRecherches.isEmpty()) {
      final long joursCommuns =
          trajet.getJoursRecurrence().stream().filter(joursRecherches::contains).count();
  ...
  private MatchingResponse toMatchingResponse(
      final Trajet trajet, final List<JourSemaine> joursRecherches) {
